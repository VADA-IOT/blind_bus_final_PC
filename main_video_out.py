...
#default resolution system dependent
cap = cv2.VideoCapture(VIDEO)
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))
video_out = cv2.VideoWriter('outpy.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 24, (frame_width,frame_height))
video_out2 = cv2.VideoWriter('outpy2.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 24, (1280, 1280))
...
def processImage(image,index):
    ...
    for i in indices:
        ...
        if (class_ids[i] == 5):
            ...
            ax,ay = (s - draw.shape[1])//2,(s - draw.shape[0])//2

            #Pasting the 'image' in a centering position
            f[ay:draw.shape[0]+ay,ax:ax+draw.shape[1]] = draw
            print(f.shape[0])
            print(f.shape[1])
           # digit_img.append(f)
            video_out2.write(f)
...
if (INPUT_IMAGE == True):
    ...
    # for j in range(0,len(location_img)):
      # out.write(location_img[j])  
     
    # out2 = cv2.VideoWriter('outpy2.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 24, (digit_img[0].shape[0],digit_img[0].shape[1]))
    # for j in range(0,len(digit_img)):
      # out2.write(digit_img[j])
